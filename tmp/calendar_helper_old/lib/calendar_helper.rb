...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    first.upto(last) do |cur|
      cell_text, cell_attrs, cell_label = block.call(cur)
      puts "-------> #{cell_text} #{cell_attrs} #{cell_label}"
      cell_text  ||= cur.mday
      cell_attrs ||= {}
      cell_label = '<br>' + cell_label[:label]
      ...
      cal << "<td #{cell_attrs}>#{cell_text}#{cell_label}</td>"
